...
namespace CustomerA.PdfConsole;
...
public sealed class RentalApiClient : IDisposable
{
    ...
    public async Task<RegisterReturnResponse> RegisterReturnAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        using var request = new HttpRequestMessage(
            HttpMethod.Post,
            $"/api/rentals/{Uri.EscapeDataString(bookingNumber)}/return")
        {
            Content = JsonContent.Create(
                new RegisterReturnRequest(
                    returnTime,
                    returnOdometer,
                    baseDailyPrice,
                    baseKmPrice),
        ...
        using var response = await client.SendAsync(request, cancellationToken);
        await EnsureSuccessAsync(response, cancellationToken);
        return await response.Content.ReadFromJsonAsync<RegisterReturnResponse>(JsonOptions, cancellationToken)
            ?? throw new InvalidOperationException("The rental API returned an empty return response.");
    }
